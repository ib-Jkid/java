...
public class Library extends Jobs {
	static Double salary = 10000.0;
	
	
	static void jobSearch () {
		if(Man.employed == false && Man.workplace != Workplace.LIBRARY) {
			System.out.printf("Welcome to the Library do you want to work here?\n"
					+ "1: Yes\n"
					+ "2: No\n");
	...
	static void work() {
		if(Man.employed == true && Man.workplace == Workplace.LIBRARY) {
			startWork();
		}else {
	...
	public static void home() {
		boolean exit = false;
		while(!exit) {
			System.out.printf("Welcome to the Library what are you here for? \n"
					+ "1: Job Search \n"
					+ "2: work \n"
					+ "3: exit \n");
			int response = getResponse(0);
	...
	private static void interview() {
		...
		if(ans >= 5) {
			...
			Man.employed = true;
			Man.workplace = Man.Workplace.LIBRARY;
		}else if(ans < 5) {
